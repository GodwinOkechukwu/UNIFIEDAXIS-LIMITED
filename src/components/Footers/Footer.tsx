...
const Footer = () => {
  ...
  return (
    <footer className=" bg-[#010F1F] font-inter  border border-[#4944541A]/10 w-full py-2 flex flex-col item-center mb-[-100px]">
          ...
          <div className="flex gap-4 w-full pt-3">
            {footerData.map((section, index) => (
              <div key={index} className="flex flex-col gap-4 lg:gap-5 w-full">
                <span className="text-[#D0BCFF] font-medium text-base leading-[1.6]">
                  {section.title}
                </span>
                {section.links.map((link, linkIndex) => (
                  <Link
                    ...
                    className="text-[#D0BCFF] text-sm leading-[1.3] hover:text-primary-100 transition-[.3]"
                  >
                    {link.label}
                  </Link>
                ))}
          ...
          <div className="flex lg:gap-8 w-full pt-3">
            {footerData.map((section, index) => (
              <div key={index} className="flex flex-col gap-2 sm:gap-5 w-full">
                <span className="text-[#D0BCFF] font-medium text-sm sm:text-base leading-[1.6]">
                  {section.title}
                </span>
                ...
                {section.links.map((link, linkIndex) => (
                  <Link
                    key={linkIndex}
                    href={link.href}
                    className={`${link.href === pathname ? "text-primary-100" : "text-[#D0BCFF]"} text-xs sm:text-sm font-[400] hover:text-[#D0BCFF]/10 transition-[.3] leading-6`}
                  >
                    {link.label}
                  </Link>
                ))}
      ...
      <div className="mx-auto max-w-[1350px]">
        <div className="flex items-center justify-center py-2">
          <div className="text-[#D0BCFF] sm:font-mono text-xs leading-[1.2]">
